...
import { CHAR_FULL_QUESTIONS, buildCharFullResult } from '../../data/charFullData'
import { trackEvent } from '../../utils/analytics'
import './index.scss'

type AnswerMap = Record<string, number>

const PAGE_KEY = 'char_full'
const HISTORY_KEY = 'campus_test_history_v2'

export default function CharFullPage() {
  ...
  const currentQuestion = CHAR_FULL_QUESTIONS[currentIndex]
  const totalQuestions = CHAR_FULL_QUESTIONS.length
  ...
  useDidShow(() => {
    ...
    if (savedAnswers && Object.keys(savedAnswers).length > 0) {
      ...
      if (autoStart) {
        trackEvent('char_full_opened', { source: 'quick-result' })
      }
    }
  ...
  const result = useMemo(() => {
    ...
    return buildCharFullResult(answers)
  }, [answers, finished])
  ...
  useShareAppMessage(() => ({
    title: result ? `🧠 我当前更接近「${result.title}」｜学生性格完整版` : '🧠 学生性格完整版：12题看看你的校园相处节奏',
    path: '/pages/char-full/index',
  }))

  useShareTimeline(() => ({
    title: result ? `🧠 ${result.shareTitle}｜学生性格完整版` : '🧠 学生性格完整版',
  }))

  const handleStart = () => {
    setStarted(true)
    trackEvent('char_full_opened', { source: 'direct' })
  }
  ...
  const handleNext = () => {
    ...
    setFinished(true)
    trackEvent('char_full_completed', { totalQuestions })
  }
  ...
  const handleRetest = () => {
    ...
    setFinished(false)
    setHistorySaved(false)
    trackEvent('char_full_retested', {})
  }

  if (!started) {
    return (
      <View className="char-full-page">
        <View className="intro-card">
          <Text className="intro-badge">v2.4-A 上新</Text>
          <Text className="intro-emoji">🧠</Text>
          <Text className="intro-title">学生性格完整版</Text>
          <Text className="intro-desc">12 道题，从表达方式、相处底色、处理方式和社交节奏四个方向，看看你当前更接近哪种校园相处风格。</Text>
          <View className="intro-tags">
            <Text className="intro-tag">12题完整版</Text>
            <Text className="intro-tag">四维性格画像</Text>
            <Text className="intro-tag">不做定论</Text>
          </View>
          <Text className="intro-note">结果更偏向你当前的相处风格和表达习惯，用来帮助观察自己，不是给你下固定结论。</Text>
      ...
      <View className="char-full-page">
        <ScrollView className="result-scroll" scrollY>
          <View className="result-card">
            <Text className="result-label">学生性格完整版</Text>
        ...
        <View className="result-actions">
          ...
          <Button
            ...
            open-type="share"
            onClick={() => trackEvent('char_full_result_share', { title: result.shareTitle })}
          >
            分享结果
          </Button>
        </View>
  ...
  return (
    <View className="char-full-page">
      <View className="test-header">
        <Text className="header-title">🧠 学生性格完整版</Text>
      ...
      <ScrollView className="question-scroll" scrollY>
        ...
        <View className="disclaimer-card">
          <Text className="disclaimer-text">结果更偏向你的当前相处风格与表达方式，不作为固定性格结论。</Text>
        </View>
