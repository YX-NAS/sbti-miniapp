...
import { LOVE_FULL_QUESTIONS, buildLoveFullResult } from '../../data/loveFullData'
import { trackEvent } from '../../utils/analytics'
import './index.scss'

type AnswerMap = Record<string, number>

const PAGE_KEY = 'love_full'
const HISTORY_KEY = 'campus_test_history_v2'

export default function LoveFullPage() {
  ...
  const currentQuestion = LOVE_FULL_QUESTIONS[currentIndex]
  const totalQuestions = LOVE_FULL_QUESTIONS.length
  ...
  useDidShow(() => {
    ...
    if (savedAnswers && Object.keys(savedAnswers).length > 0) {
      ...
      if (autoStart) {
        trackEvent('love_full_opened', { source: 'quick-result' })
      }
    }
  ...
  const result = useMemo(() => {
    ...
    return buildLoveFullResult(answers)
  }, [answers, finished])
  ...
  useShareAppMessage(() => ({
    title: result ? `💕 我当前更接近「${result.title}」｜情感模式完整版` : '💕 情感模式完整版：12题看看你当前更接近哪种靠近方式',
    path: '/pages/love-full/index',
  }))

  useShareTimeline(() => ({
    title: result ? `💕 ${result.shareTitle}｜情感模式完整版` : '💕 情感模式完整版',
  }))

  const handleStart = () => {
    setStarted(true)
    trackEvent('love_full_opened', { source: 'direct' })
  }
  ...
  const handleNext = () => {
    ...
    setFinished(true)
    trackEvent('love_full_completed', { totalQuestions })
  }
  ...
  const handleRetest = () => {
    ...
    setFinished(false)
    setHistorySaved(false)
    trackEvent('love_full_retested', {})
  }

  if (!started) {
    return (
      <View className="love-full-page">
        <View className="intro-card">
          <Text className="intro-badge">v2.4-A 上新</Text>
          <Text className="intro-emoji">💕</Text>
          <Text className="intro-title">情感模式完整版</Text>
          <Text className="intro-desc">12 道题，从表达方式、靠近边界、互动频率和关系节奏四个方向，看看你当前更接近哪种情感靠近模式。</Text>
          <View className="intro-tags">
            <Text className="intro-tag">12题完整版</Text>
            <Text className="intro-tag">四维关系画像</Text>
            <Text className="intro-tag">不做定论</Text>
          </View>
          <Text className="intro-note">结果更偏向你当前阶段在关系里的靠近方式和安全感模式，用来帮助观察自己，不是给你贴固定标签。</Text>
      ...
      <View className="love-full-page">
        <ScrollView className="result-scroll" scrollY>
          <View className="result-card">
            <Text className="result-label">情感模式完整版</Text>
        ...
        <View className="result-actions">
          ...
          <Button
            ...
            open-type="share"
            onClick={() => trackEvent('love_full_result_share', { title: result.shareTitle })}
          >
            分享结果
          </Button>
        </View>
  ...
  return (
    <View className="love-full-page">
      <View className="test-header">
        <Text className="header-title">💕 情感模式完整版</Text>
      ...
      <ScrollView className="question-scroll" scrollY>
        ...
        <View className="disclaimer-card">
          <Text className="disclaimer-text">结果更偏向你的当前靠近方式与关系节奏，不作为固定情感类型结论。</Text>
        </View>
