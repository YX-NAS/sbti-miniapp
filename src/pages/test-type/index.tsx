...
export default function TestType() {
  ...
  const handleResume = (item: { path: string; id: string; title: string }) => {
    trackEvent('test_center_resume_click', { entryId: item.id, title: item.title })
    Taro.navigateTo({ url: item.path })
  }
    ...
    <View className="test-type-container">
      ...
      <View className="insight-strip">
        <View className="streak-card">
          <Text className="streak-num">{visitStreak.current || 1}</Text>
          <View className="streak-copy">
            <Text className="streak-title">连续访问天数</Text>
            <Text className="streak-desc">用快测做钩子，完整版更适合慢慢回看自己</Text>
          </View>
        </View>
      </View>
      ...
      {resumeCards.length > 0 && (
        <>
          <View className="section-title-row">
            <Text className="section-title">⏳ 继续上次完整版</Text>
          </View>
          <View className="resume-list">
            {resumeCards.map(item => (
              <View key={item.id} className="resume-card" onClick={() => handleResume(item)}>
                <View className="resume-top">
                  <Text className="resume-emoji">{item.emoji}</Text>
                  <Text className="resume-progress">{item.progressText}</Text>
                </View>
                <Text className="resume-title">{item.title}</Text>
                <View className="resume-bar">
                  <View className="resume-fill" style={{ width: `${item.progress}%`, background: item.color }} />
                </View>
                <Text className="resume-cta" style={{ color: item.color }}>继续测试 →</Text>
              </View>
            ))}
          </View>
        </>
