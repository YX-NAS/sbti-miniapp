...
import { applySuggestedHotRotation, getSuggestedHotRotation } from '../../utils/recommendations'
...
export default function AdminConsole() {
  ...
  const analyticsEntries = Object.entries(analyticsSummary)
    ...
    .slice(0, 8)
  const suggestedHotTests = getSuggestedHotRotation(config.hotTests)
  const visitStreak = getVisitStreak()
  const resumeTests = getResumeFullTests()
  const funnelRank = [...funnelStats].sort((a, b) => {
    const aRate = a.completionRate === '-' ? -1 : Number(a.completionRate.replace('%', ''))
    const bRate = b.completionRate === '-' ? -1 : Number(b.completionRate.replace('%', ''))
    return bRate - aRate
  })
  ...
  const handleApplyHotRotation = () => {
    persistConfig({
      ...config,
      hotTests: applySuggestedHotRotation(config.hotTests),
    })
    Taro.showToast({ title: '已应用今日热推顺序', icon: 'success' })
  }
  ...
  return (
    <ScrollView className="admin-console" scrollY>
      ...
      <View className="admin-section">
        <Text className="section-title">运营建议</Text>
        <View className="summary-grid">
          <View className="summary-chip">
            <Text className="summary-label">连续访问</Text>
            <Text className="summary-value">{visitStreak.current || 1} 天</Text>
          </View>
          <View className="summary-chip">
            <Text className="summary-label">待续测完整版</Text>
            <Text className="summary-value">{resumeTests.length}</Text>
          </View>
        </View>
        <View className="ops-card">
          <Text className="ops-title">今日建议热推顺序</Text>
          {suggestedHotTests.map(item => (
            <View key={item.id} className="ops-row">
              <Text className="ops-name">{item.emoji} {item.title}</Text>
              <Text className="ops-tag">{item.tag}</Text>
            </View>
          ))}
          <View className="action-btn" onClick={handleApplyHotRotation}>一键应用今日热推</View>
        </View>
        {funnelRank.length > 0 && (
          <View className="ops-card">
            <Text className="ops-title">完整版完成率领先</Text>
            {funnelRank.slice(0, 2).map(item => (
              <View key={item.type} className="ops-row">
                <Text className="ops-name">{item.type}</Text>
                <Text className="ops-tag">{item.completionRate}</Text>
              </View>
            ))}
          </View>
        )}
