...
import { STUDY_FULL_QUESTIONS, buildStudyFullResult } from '../../data/studyFullData'
import { trackEvent } from '../../utils/analytics'
import './index.scss'

type AnswerMap = Record<string, number>

const PAGE_KEY = 'study_full'
const HISTORY_KEY = 'campus_test_history_v2'

export default function StudyFullPage() {
  ...
  const currentQuestion = STUDY_FULL_QUESTIONS[currentIndex]
  const totalQuestions = STUDY_FULL_QUESTIONS.length
  ...
  useDidShow(() => {
    ...
    if (savedAnswers && Object.keys(savedAnswers).length > 0) {
      ...
      if (autoStart) {
        trackEvent('study_full_opened', { source: 'quick-result' })
      }
    }
  ...
  const result = useMemo(() => {
    ...
    return buildStudyFullResult(answers)
  }, [answers, finished])
  ...
  useShareAppMessage(() => ({
    title: result
      ? `📚 我当前更接近「${result.title}」｜学习状态完整版`
      : '📚 学习状态完整版：12题看看你更适合哪种学习推进方式',
    path: '/pages/study-full/index',
  }))

  useShareTimeline(() => ({
    title: result ? `📚 ${result.shareTitle}｜学习状态完整版` : '📚 学习状态完整版',
  }))

  const handleStart = () => {
    setStarted(true)
    trackEvent('study_full_opened', { source: 'direct' })
  }
  ...
  const handleNext = () => {
    ...
    trackEvent('study_full_answered', {
    ...
    setFinished(true)
    trackEvent('study_full_completed', { totalQuestions })
  }
  ...
  const handleRetest = () => {
    ...
    setFinished(false)
    setHistorySaved(false)
    trackEvent('study_full_retested', {})
  }

  if (!started) {
    return (
      <View className="study-full-page">
        <View className="intro-card">
          <Text className="intro-badge">v2.4-A 首发</Text>
          <Text className="intro-emoji">📚</Text>
          <Text className="intro-title">学习状态完整版</Text>
          <Text className="intro-desc">12 道题，从启动节奏、任务组织、学习陪伴和处理重点四个方向，看看你当前更接近哪种学习推进方式。</Text>
          <View className="intro-tags">
            <Text className="intro-tag">12题完整版</Text>
            <Text className="intro-tag">四维学习画像</Text>
            <Text className="intro-tag">不做定论</Text>
          </View>
          <Text className="intro-note">结果更偏向你当前阶段的学习习惯与推进方式，用来帮助观察自己，不是给你下固定标签。</Text>
      ...
      <View className="study-full-page">
        <ScrollView className="result-scroll" scrollY>
          <View className="result-card">
            <Text className="result-label">学习状态完整版</Text>
        ...
        <View className="result-actions">
          ...
          <Button
            ...
            open-type="share"
            onClick={() => trackEvent('study_full_result_share', { title: result.shareTitle })}
          >
            分享结果
          </Button>
        </View>
  ...
  return (
    <View className="study-full-page">
      <View className="test-header">
        <Text className="header-title">📚 学习状态完整版</Text>
      ...
      <ScrollView className="question-scroll" scrollY>
        ...
        <View className="disclaimer-card">
          <Text className="disclaimer-text">结果更偏向你的当前学习节奏与推进方式，不作为固定类型结论。</Text>
        </View>
